 ...
 * @returns
 */
// Create middleware to protect products, only owners should be able to update/delete
 ...
 */
// Create middleware to protect products, only owners should be able to update/delete
const protectCategoriesOwners = (req, res, next) => {
  const { sessionUser, categorie } = req;

  if (sessionUser.id !== categorie.id) {
    return res.status(403).json({
      status: "error",
